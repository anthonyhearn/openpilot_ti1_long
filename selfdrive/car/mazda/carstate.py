...
class CarState(CarStateBase):
  ...
  def update_gen2(self, cp, cp_cam, cp_body, frogpilot_variables):
    ...
    self.accel_button = cp.vl["CRZ_BTNS"]["RES"] if "CRZ_BTNS" in cp.vl else 0
    self.decel_button = cp.vl["CRZ_BTNS"]["SET_M"] if "CRZ_BTNS" in cp.vl else 0
    self.cancel_button = cp.vl["CRZ_BTNS"]["CAN_OFF"] if "CRZ_BTNS" in cp.vl else 0
    self.main_button = int(cp.vl["CRZ_BTNS"]["MODE_X"] == 1 and cp.vl["CRZ_BTNS"]["MODE_Y"] == 1) if "CRZ_BTNS" in cp.vl else 0

    self.cp = cp
    self.cp_cam = cp_cam
    self.acc = copy.copy(cp.vl["ACC"] if "ACC" in cp.vl else {})
